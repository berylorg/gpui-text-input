...
#[gpui::test]
fn deferred_processing_rejection_preserves_custody_for_exact_retry(cx: &mut gpui::TestAppContext) {
    let (input, cx) = cx.add_window_view(|window, cx| {
        RangeTextInput::new(config(2 * 1024 * 1024, 32_768), window, cx).unwrap()
    });
    let key = stage_terminal_target_object_response(&input, cx, SOURCE);
    cx.update(|window, app| {
        input.update(app, |input, cx| {
            ...
            let candidate = input.surface_candidate.take();
            input.begin_realization_frame();
            assert!(matches!(
            ...
            input.surface_candidate = candidate;
            input.begin_realization_frame();
            assert!(
                input
                    .service_deferred_geometry_response(window, cx)
                    .unwrap()
            );
            assert!(input.deferred_geometry_response.is_none());
            assert!(!input.dispatched_object_pages.contains(&key));
